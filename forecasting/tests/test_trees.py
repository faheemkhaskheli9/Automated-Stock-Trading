"""Round-trip and leakage-guard tests for the gradient-boosted tree predictor.

Mirrors ``test_linear.py``: the guard contract is shared machinery
(``forecasting._frame_model``), so the same scenarios must hold here.
"""
...
from forecasting.trees import GradientBoostingPredictor
...
def test_registration(predictor):
    assert "gradient_boosting" in set(registered_keys())
    assert get_predictor_class("gradient_boosting") is GradientBoostingPredictor
    assert predictor.key == "gradient_boosting"
...
def test_predict_series_round_trip(history, predictor):
    ...
    assert [p.model_key for p in result] == ["gradient_boosting"] * 3
...
def test_tiny_learning_rate_collapses_to_return_mean(history):
    predictor = GradientBoostingPredictor(learning_rate=1e-8, max_iter=1)
    predictor.fit(history)
    mean_return = float(
    ...
    for prediction, close_now in zip(predictor.predict_series(frame), [140.0, 175.0]):
        assert prediction.predicted_close == pytest.approx(
            close_now * (1.0 + mean_return), rel=1e-4
        )
...
def test_nan_features_are_tolerated_without_an_imputer(history, predictor):
    """HistGradientBoosting handles NaNs natively; no pipeline imputer stage."""
    with_gap = deepcopy(history)
    with_gap.inputs.X.iloc[:3, with_gap.inputs.X.columns.get_loc("tech.momentum")] = np.nan
    predictor.fit(with_gap)
    assert [step[0] for step in predictor._pipeline.steps] == ["model"]
    frame = follow_on(history, [161.0, 162.5], momentum=[np.nan, 1.5])
    result = predictor.predict_series(frame)
    assert len(result) == 2
    assert all(p.predicted_close > 0 for p in result)
...
def test_fit_and_predict_over_assembled_frames(db, predictor):
    ...
    assert prediction.model_key == "gradient_boosting"
