...
def test_stacking_ensemble_fits_and_predicts():
    rng = np.random.RandomState(0)
    X = rng.normal(size=(60, 3))
    y = X[:, 0] * 2.0 - X[:, 1] + rng.normal(scale=0.01, size=60)

    est = get_estimator("stacking_ensemble").build(estimators="ridge,hist_gbr", cv=3)
    est.fit(X, y)
    preds = est.predict(X)
    assert preds.shape == (60,)
    assert np.corrcoef(preds, y)[0, 1] > 0.5
...
def test_stacking_ensemble_rejects_single_estimator():
    with pytest.raises(ValueError):
        get_estimator("stacking_ensemble").build(estimators="ridge")


def test_stacking_ensemble_rejects_self_reference():
    with pytest.raises(ValueError):
        get_estimator("stacking_ensemble").build(estimators="ridge,stacking_ensemble")


def test_stacking_ensemble_rejects_baseline_sub_estimator():
    with pytest.raises(ValueError):
        get_estimator("stacking_ensemble").build(estimators="ridge,naive_last")


def test_stacking_ensemble_rejects_classifier_sub_estimator():
    with pytest.raises(ValueError):
        get_estimator("stacking_ensemble").build(estimators="ridge,logistic")


def test_stacking_ensemble_rejects_bad_final_estimator():
    with pytest.raises(ValueError):
        get_estimator("stacking_ensemble").build(
            estimators="ridge,hist_gbr", final_estimator="naive_last"
        )
    with pytest.raises(ValueError):
        get_estimator("stacking_ensemble").build(
            estimators="ridge,hist_gbr", final_estimator="stacking_ensemble"
        )


def test_stacking_ensemble_rejects_low_cv():
    with pytest.raises(ValueError):
        get_estimator("stacking_ensemble").build(estimators="ridge,hist_gbr", cv=1)
