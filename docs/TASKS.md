...
Delivered on two tracks:

1. The standalone **`backtesting` app** over `modeling.TradingModel` (routed
   `/backtests/`). Full UI + admin + `backtest_model` command + unscheduled
   Celery tasks. 44 tests. See `docs/BACKTESTING.md`. A `Backtest` runs in
   one of two `fit_mode`s: `walk_forward` (retrain each fold) or
   `frozen_artifact` (score a `modeling`-trained joblib artifact over every
   session after `trained_at`).
...
| # | Task | Status | Notes |
|---|------|--------|-------|
| C1 | `backtesting/walkforward.py`: `walk_forward()` expanding/rolling schemes, fresh predictor per fold | ✅ | `generate_folds()` (pure) + `engine.run_backtest` builds a fresh `modeling.training.build_pipeline` per fold. **Strict path**: `forecasting/backtesting/walkforward.py::generate_folds` (row-position windows) + `engine.walk_forward` instantiates the predictor class per fold, `fit()` on that slice only |
...
| C3 | `backtesting/metrics.py`: MAE/RMSE/MAPE/R², directional accuracy, skill-vs-naive, trading translation | ✅ | Accuracy reuses `modeling.metrics`; `metrics.positions_from_forecast` + `simulate_instrument` + `combine_equity_curves` do the trade translation. **Strict path**: `forecasting/backtesting/metrics.py::regression_scores` + `trading_translation` (long-if-up, cost-bps, equity curve) |
...
| C5 | Leak canary test: future-peeking predictor is flagged by the harness | ✅ | `test_engine.py::test_naive_baseline_is_beatable_reference` (naive skill-vs-naive ~= 0); `test_leakage.py` availability asserts. **Strict path**: `test_walk_forward.py::test_future_peeking_predictor_is_flagged_as_leaky` — a predictor that reads the target close from the DB trips `engine.looks_leaky`; `naive` does not |
...
| D5 | Symbol detail — news headlines + sentiment chips; social/fundamentals "not configured" panels | ✅ | `marketdata.views._research_panels` reads the `research` app's raw tables point-in-time (`published_at`/`posted_at`/`as_of_report_date` ≤ now): latest 12 `NewsItem` headlines with a VADER sentiment chip + 7d/30d counts and mean/trend (via `research.providers.news.news_features`); `SocialMention` and `CompanyFundamental` render a summary when rows exist, else an explanatory "Not configured" panel (their live ingestion is deferred). 11 new tests |
...
| D7 | Backtest runner — form → HTMX POST → per-fold table, skill badge, predicted-vs-actual chart, equity curve; persist `BacktestRun` | 🟡 | The standalone `backtesting` app already delivers this over `modeling.TradingModel` (config form, **Run backtest**, per-fold table, skill-vs-naive, predicted-vs-actual + equity SVGs, CSV, persisted `BacktestRun`) — full-page POST, not HTMX. Now reachable from the shared menubar as **Model Backtests** (`backtesting:index`); the old link is relabelled **Strategy Backtest**. Remaining: HTMX partial refresh polish |
| D8 | Accuracy leaderboard — rank active models by rolling MAE / directional acc / skill score | ✅ | `/modeling/leaderboard/` (`modeling/leaderboard.py::build_leaderboard`). Ranks active `TradingModel`s over a trailing 30/90/180/365-day window of backfilled `ModelPrediction` rows: MAE (target units; N/A for the `direction` classifier), directional accuracy vs the pre-forecast close, skill vs the naive no-change forecast. Ranked by skill, tie-broken by directional accuracy. Server-rendered, in the shared menubar as **Leaderboard**. Tests in `modeling/tests/test_leaderboard.py`; see `docs/MODELING.md` |
...
- 2026-09-06 - Phase C for the strict `forecasting` path: new
  `forecasting/backtesting/` package. `walkforward.generate_folds` (pure,
  row-position windows, expanding/rolling, `gap` embargo, `max(train) <
  min(test)` asserted per fold); `engine.walk_forward` assembles the
  point-in-time `TrainingFrame` once, then per fold instantiates the
  predictor class fresh, drops training rows whose label wasn't observable
  before the fold's first test decision, `fit()`s on that slice only, and
  scores a label-free `PredictionFrame` - returns a `WalkForwardResult`
  (per-fold `FoldReport`s, pooled predictions/metrics/`naive_metrics`/
  `trading`, `skipped_folds`). `metrics.regression_scores` (MAE/RMSE/MAPE/
  R2 + directional accuracy + skill-vs-naive) and `metrics.trading_translation`
  (long-if-up, cost bps, equity curve, Sharpe/drawdown/hit rate).
  `engine.looks_leaky` heuristic canary + `backtest_predictor` management
  command. `sarima`/`ets` folds are skipped unless `--gap 0` (prefix-replay
  contract). No result persistence / dashboard runner for this path yet.
  10 tests (`forecasting/tests/test_walk_forward.py`: 40-config fold-
  disjointness fuzz, metrics units, naive/ridge end-to-end, future-peeking
  predictor flagged); `forecasting` suite 64 passed / 6 skipped, full suite
